...
describe('resolveVerifyOptions', () => {
  ...
  it('--repo defaults to the given cwd when omitted (locked devex-review default)', () => {
    const options = resolveVerifyOptions(parseCliFlags(['--proxy', 'rtk']), cwd);
    expect(options.repo).toBe(cwd);
  });

  it('--tasks defaults to the bundled corpus path when omitted (locked devex-review default)', () => {
    const options = resolveVerifyOptions(parseCliFlags(['--proxy', 'rtk']), cwd);
