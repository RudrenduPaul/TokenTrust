 ...
 * finding (CLAUDE.md anti-sycophancy rule 2). `rtk`'s figure is the one
 * cited in the TokenTrust strategy doc's Product Definition section
 * ("up to 70% context reduction"). `headroom` and `lean-ctx` have no sourced
 * figure captured yet as of this build pass -- the benchmark + README
 * pipeline step that follows this one is responsible for sourcing and
 * verifying those before publishing any comparison that implies a number
 * for them.
 */
export const CLAIMED_SAVINGS: Record<ProxyName, ClaimedSavings> = {
  ...
  'lean-ctx': { pct: null, label: 'no claimed figure on file' },
};
